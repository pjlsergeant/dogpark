...
export interface Pages<T, P extends Page<T>> {
  ...
  /**
   * Later pages have been loaded, or the first of them is on its way. A
   * refresh discards them — an in-flight one by moving the generation under
   * it, so the person's explicit "load more" would silently produce nothing.
   * A screen that refreshes itself on a change signal holds off while this
   * is true — the same rule the Reader's poll keys on — and leaves it to
   * the person's own Refresh.
   */
