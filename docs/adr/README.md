...
| [0004](0004-messages-are-immutable.md) | Messages are immutable | `src/store/store.test.ts` :: "reads back what was posted, however the thread is read"; `src/store/store.test.ts` :: "renders a message as it read at the time, from the label history" — enforced at the API surface only: the schema has no UPDATE trigger, so raw SQL could still rewrite a body |
