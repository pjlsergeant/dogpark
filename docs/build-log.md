...
* `src/store/` — SQLite: schema, migrations, and the domain queries. The one
  ...
  `records.ts` is the public surface (shapes and the `Store` interface);
  `index.ts` composes one module per domain (`agents`, `spaces`,
  `conversations`, `messages`, `read-log`, `escalations`, `sessions`) over
  `context.ts` — the connection, the prepared statements in `statements.ts`,
  the clock, and the lookups more than one module shares. `render.ts` turns
  rows into `Message`s.
